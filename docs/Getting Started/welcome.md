...
<Cards>
  ...
  <Card title="4. Revoke Session" href="/recipes/authentication" icon="fa-duotone fa-right-from-bracket">Call `DELETE /entrypoint/org/v1/sessions/{id}` on logout or job completion.</Card>
</Cards>
...
<Cards>
  <Card title="API Session" href="/recipes/authentication" icon="fa-duotone fa-server">ClientId + ClientSecret → sessionId → call endpoints with session headers → revoke on logout.</Card>
